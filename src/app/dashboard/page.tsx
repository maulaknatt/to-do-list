...
import { Plus, Search, Filter, LayoutDashboard, Settings, LogOut, CheckCircle, X, Loader2, Calendar, Tag, Trash2, Edit3 } from 'lucide-react';
...
export default function DashboardPage() {
  ...
  const handleUpsertCategory = async (e: React.FormEvent) => {
    ...
    if (editingCat) {
      ...
      if (!error) {
        setCategories(categories.map(c => c.id === editingCat.id ? { ...c, name: newCatName, color: newCatColor } : c));
  ...
  const handleDeleteCategory = async (id: number) => {
    if (confirm('Delete category? Tasks in this category will become uncategorized.')) {
      ...
      if (!error) {
        ...
        setTasks(tasks.map(t => t.category?.id === id ? { ...t, category: undefined } : t));
      }
    }
  };
      ...
      <main className="flex-1 pl-64">
        <div className="mx-auto max-w-5xl p-10">
          ...
          <div className="mb-8 flex items-center gap-4">
            ...
            <select value={filterCat} onChange={(e) => setFilterCat(e.target.value)} className="rounded-2xl border border-white/5 bg-zinc-900 px-4 py-3 font-bold text-zinc-400"><option value="">All Categories</option>{categories.map(cat => <option key={cat.id} value={cat.id}>{cat.name}</option>)}</select>
          </div>
